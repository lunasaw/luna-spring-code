...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		// 请跟踪当前（父）委托，该委托可以为null。创建一个新的（子）委托，并带有对父参考的引用以进行回退，
		// 然后最终将this.delegate重设回其原始（父）参考。此行为模拟了一组委托，而实际上没有必要。
		...
		if (this.delegate.isDefaultNamespace(root)) {
			...
			if (StringUtils.hasText(profileSpec)) {
				...
				// 我们不能使用Profiles.of(…)，因为不支持配置文件表达式
	...
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		...
		// 创建实例 bdHolder：首先委托 BeanDefinitionParserDelegate 类的 parseBeanDefinitionElement 方法进行元素解析，
		// 经过解析后，bdHolder 实例已经包含刚才我们在配置文件中设定的各种属性，例如 class、 id、 name、 alias等属性。
		BeanDefinitionHolder bdHolder = delegate.parseBeanDefinitionElement(ele);
		if (bdHolder != null) {
			bdHolder = delegate.decorateBeanDefinitionIfRequired(ele, bdHolder);
			try {
				// Register the final decorated instance. （注释 1.16 注册最后修饰后的实例）
				// 注册 bdHolder 信息：解析完成了，需要往容器的 beanDefinitionMap 注册表注册 bean 信息，
				// 注册操作委托给了 BeanDefinitionReaderUtils.registerBeanDefinition，通过工具类完成信息注册。
