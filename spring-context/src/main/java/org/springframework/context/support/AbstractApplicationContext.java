...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 读取bean的定义信息 并创建bean工厂 类的注册到 bean factory 也是在这一步
			// （DefaultListableBeanFactory# beanDefinitionMap 和 beanDefinitionNames）
	...
	/**
	 * 配置工厂的标准上下文特征，例如上下文的ClassLoader和后处理器。 对bean工厂的初始化
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 如果之前没有任何bean后处理器（例如，PropertyPlaceholderConfigurer Bean）进行任何注册，请注册一个默认的嵌入式值解析器：此时，主要用于注释属性值的解析。
	 ...
	 * 如果实现ConfigurableApplicationContext，则返回父上下文的内部bean工厂；否则，返回false。 否则，返回父上下文本身
